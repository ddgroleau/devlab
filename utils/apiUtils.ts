...
export const resolveResponse = (status?:number) => {
    ...
    case 400:
        return {success: false, message: ServerResponses.BadRequest};
    case 404 :
...
export const apiRequest = async <T>({ path, headers, redirectPath, cacheOption = CacheOptions.Indefinite, cacheTimeToLive}:ApiRequest):Promise<T> => {
    ...
    const res = await fetch(baseApiUri + path, {method: "GET", headers: headers, next: {}});
    const resolvedResponse = resolveResponse(res.status);
